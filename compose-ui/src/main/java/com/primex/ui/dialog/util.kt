...
import com.primex.ui.R

private val REGISTRY_OWNER_KEY = R.id.view_tree_saved_state_registry_owner
 ...
 */
@JvmName("set")
internal fun View.setViewTreeSavedStateRegistryOwner(owner: SavedStateRegistryOwner?) {
    setTag(REGISTRY_OWNER_KEY, owner)
}
 ...
 */
@JvmName("get")
internal fun View.findViewTreeSavedStateRegistryOwner(): SavedStateRegistryOwner? {
    return generateSequence(this) { view ->
        view.parent as? View
    }.mapNotNull { view ->
        view.getTag(REGISTRY_OWNER_KEY) as? SavedStateRegistryOwner
    }.firstOrNull()
}